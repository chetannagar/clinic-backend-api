...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        availability = [
            DoctorAvailabilityFactory.create(doctor=random.choice(doctors))
            for _ in range(2)
        ]
        clinic_settings = ClinicSettingFactory.create_batch(3)
        reviews = ReviewFactory.create_batch(
            15,
            patient=factory.LazyFunction(lambda: random.choice(patients)),
            doctor=factory.LazyFunction(lambda: random.choice(doctors)),
        )
        audit_logs = [AuditLogFactory(user=random.choice(users)) for _ in range(20)]
        ...
        notifications = NotificationFactory.create_batch(
            30,
            user=factory.LazyFunction(lambda: random.choice(users)),
            appointment=factory.LazyFunction(
                lambda: random.choice(appointments) if appointments else None
            ),
        )
        messages = MessageFactory.create_batch(
            20,
            sender=factory.LazyFunction(lambda: random.choice(users)),
            receiver=factory.LazyFunction(lambda: random.choice(users)),
        )
        payments = PaymentFactory.create_batch(
            10,
            patient=factory.LazyFunction(lambda: random.choice(patients)),
            doctor=factory.LazyFunction(lambda: random.choice(doctors)),
            appointment=factory.LazyFunction(lambda: random.choice(appointments)),
        )
        ...
        prescriptions = PrescriptionFactory.create_batch(
            10,
            patient=factory.LazyFunction(lambda: random.choice(patients)),
            doctor=factory.LazyFunction(lambda: random.choice(doctors)),
        )
        ...
        self.stdout.write(
            self.style.SUCCESS("Successfully populated database with dummy data") # pylint: disable=no-member
        )
